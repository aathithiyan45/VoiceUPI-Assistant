...
/**
 * ConfirmationActivity — Voice-first payment confirmation.
 *
 * Receives via Intent:
 *   EXTRA_MERCHANT_NAME  : String
 *   EXTRA_UPI_ID         : String
 *   EXTRA_AMOUNT         : String  (e.g. "100.0")
 *
 * Voice flow:
 *   TTS: "You are paying ₹{amount} to {merchant}. Say yes to confirm or no to cancel."
 *   "yes" → SuccessActivity
 *   "no"  → VoiceMainActivity (finish confirmation)
 */
class ConfirmationActivity : AppCompatActivity() {
    ...
    private fun setupTts() {
        tts = TextToSpeech(this) { status ->
            if (status == TextToSpeech.SUCCESS) {
                ...
                // Announce payment details and prompt
                speakConfirmation()
            } else {
    ...
    private fun confirmPayment() {
        setStatus("Confirmed! Processing…")
        speak("Confirming payment.", UTT_RESULT)
        handler.postDelayed({
